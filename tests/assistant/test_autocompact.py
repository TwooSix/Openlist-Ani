...
class TestPartialCompactPreservesRecent:
    """Test that partial_compact summarizes old messages and keeps recent ones."""
    ...
    async def test_partial_compact_preserves_recent_messages(self):
        ...
        messages = [
            ...
            Message(role=Role.USER, content="Old question about design"),
            Message(role=Role.ASSISTANT, content="Old answer about design"),
            Message(role=Role.USER, content="Another old question"),
            Message(role=Role.ASSISTANT, content="Another old answer"),
            # Recent messages that should survive
            Message(role=Role.USER, content="Recent: please fix the bug"),
            Message(role=Role.ASSISTANT, content="Recent: I fixed the bug"),
        ]

        # preserved_tail=2 keeps the last 2 non-system messages
        result = await compactor.partial_compact(messages, preserved_tail=2)

        assert result is not None
        # System message should be the first
        assert result[0].role == Role.SYSTEM
        assert result[0].content == "You are a helpful assistant."

        # The tail should contain the recent messages verbatim
        tail_contents = [m.content for m in result]
        assert "Recent: please fix the bug" in tail_contents
        assert "Recent: I fixed the bug" in tail_contents
    ...
    async def test_partial_compact_with_explicit_pivot(self):
        ...
        messages = [
            Message(role=Role.SYSTEM, content="System prompt"),
            Message(role=Role.USER, content="Msg A"),
            Message(role=Role.ASSISTANT, content="Reply A"),
            Message(role=Role.USER, content="Msg B"),
            Message(role=Role.ASSISTANT, content="Reply B"),
        ]

        # pivot_index=3: head=[system, Msg A, Reply A], tail=[Msg B, Reply B]
        result = await compactor.partial_compact(messages, pivot_index=3)

        assert result is not None
        tail_contents = [m.content for m in result]
        # Tail messages preserved
        assert "Msg B" in tail_contents
        assert "Reply B" in tail_contents
        # Old messages replaced by summary
        assert "Msg A" not in tail_contents
        assert "Reply A" not in tail_contents


class TestReadFileTrackerPostCompactRestoration:
    """Track files, compact, verify restoration injects file contents."""
    ...
    async def test_tracked_files_restored_after_compact(self):
        ...
        assert result is not None
        # Expect: system + summary + file restoration
        assert len(result) == 3
        assert result[0].role == Role.SYSTEM
        ...
        assert "DEBUG = True" in file_msg.content
    ...
    async def test_no_restoration_with_empty_tracker(self):
        ...
        messages = [
            Message(role=Role.SYSTEM, content="System"),
            Message(role=Role.USER, content="w" * 3000),
        ]
        result = await compactor.maybe_compact(messages)
