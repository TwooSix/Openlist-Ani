"""
Release download priority filtering.
...
from openlist_ani.application.anime_library_ingestion.settings import PrioritySettings
...
class PriorityFilter:
    """Filters a batch of parsed releases according to priority rules."""
    ...
    async def apply(
        ...
        if skipped:
            logger.debug(
                f"Priority filter: {len(accepted)} accepted, {skipped} skipped"
            )
        return accepted
    ...
    async def _filter_group(
        ...
        for candidate in group:
            ...
            # Priority check: skip if a better release is already downloaded
            # or pending download.
            if known and self._should_skip_by_db(
                ...
                logger.debug(
                    ...
                    f"(higher-priority release already downloaded)"
                )
                continue
            ...
            records.append(
                {
                    ...
                    "languages": "".join(lang.value for lang in release.languages),
