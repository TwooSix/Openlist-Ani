...
class SkillCatalog:
    ...
    def get_skill_content(self, name: str) -> str | None:
        """Return the full SKILL.md body content for a named skill.

        The content is the resolved body (after YAML frontmatter),
        with @include directives expanded. Returns None if the skill
        is not found or has no body content.
        """
        skill = self._skills.get(name)
        if skill is None:
            return None
        return skill.body_content or None
    ...
    def _format_skill_entry(self, skill: SkillEntry) -> str:
        ...
        if skill.actions:
            ...
            lines.append("Type: guide (invoke to load workflow instructions)")
        lines.append("")
