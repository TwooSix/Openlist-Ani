...
class MemoryTool(BaseTool):
    ...
    def parameters(self) -> dict:
        return {
            ...
            "properties": {
                ...
                "name": {
                    "type": "string",
                    "description": ("Frontmatter 'name' field (for write)."),
                },
                "type": {
                    "type": "string",
                    "enum": [
                ...
                "description": {
                    "type": "string",
                    "description": ("Frontmatter 'description' field (for write)."),
                },
                "old_str": {
                    "type": "string",
                    "description": (
                        "Text to find (for update). Must be unique in the file."
                    ),
                },
                "new_str": {
                    "type": "string",
    ...
    def prompt(
        ...
        return "\n".join(
            [
                "# Memory Tool",
                "",
                "Use the `memory` tool to manage your persistent memory files.",
                "",
                "## Available Actions",
                "",
                ...
                "| `update_index` | Overwrite MEMORY.md index | `content` |",
                "",
    ...
    async def _action_read(self, kwargs: dict) -> str:
        ...
        if not content:
            return f"Error: Memory file '{filename}' does not exist."
        return content
    ...
    async def _action_delete(self, kwargs: dict) -> str:
        ...
        filename = str(kwargs.get("filename", ""))
        if not filename:
            return "Error: 'filename' is required for the delete action."
    ...
    async def _action_update(self, kwargs: dict) -> str:
        ...
        filename = str(kwargs.get("filename", ""))
        if not filename:
            return "Error: 'filename' is required for the update action."
    ...
    async def _action_update_index(self, kwargs: dict) -> str:
        ...
        if content is None or str(content).strip() == "":
            return "Error: 'content' is required for the update_index action."
