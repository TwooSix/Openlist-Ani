...
3. Memory       -- Directory-based memory system prompt + index
4. Skill catalog listing
5. Environment info
"""
...
class ContextBuilder:
    ...
    Uses the directory-based memory system:
    - SOUL.md as the base system prompt
    - memory/ directory prompt (instructions + MEMORY.md index)
    ...
    def build_system(self) -> list[Message]:
        ...
        # 3. Memory system prompt (instructions + MEMORY.md index)
        memory_prompt = self._memory.build_memory_prompt()
        if memory_prompt.strip():
            system_parts.append(memory_prompt.strip())
        ...
        # 5. Per-tool prompt contributions
